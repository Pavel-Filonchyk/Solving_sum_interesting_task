...
export default function Authors({item, items}: any) {
    const find = items.filter((elem: any) => elem.id === item.author)
    const findAuthor = find.map((data: any) =>{ 
        return (
            <div className="name_author">{data.name}</div>
        )
    })
    const findAvatar = find.map((data: any) =>{ 
        return (
            <img src={data.avatar} alt="" style={{width: "150px", height: "150px"}}/>
        )
    })
